...
inline mypair GetForwardHash(int l ,int r , int len)
{
	if(l == 1)
		return HashF[r];
	pair<int,int> hashp;
	hashp.first = (HashF[r].l - 1ll * HashF[l-1].l * pwr(prime[0],r-l+1,mod[0]) ) % mod[0];
	if(hashp.first < 0)
		hashp.first += mod[0];

	hashp.second = (HashF[r].r- 1ll * HashF[l-1].r * pwr(prime[1],r-l+1,mod[1]) ) % mod[1];
	if(hashp.second < 0)
		hashp.second += mod[1];
	mypair FH;
	FH.l = hashp.first;
...
inline mypair GetReverseHash(int l ,int r , int len)
{
	if(r == len)
		return HashR[l];
	pair<int,int> hashp;
	hashp.first= (HashR[l].l - 1ll * HashR[r+1].l* pwr(prime[0],r-l+1,mod[0]) ) % mod[0];
	if(hashp.first< 0)
		hashp.first+= mod[0];
	hashp.second= (HashR[l].r- 1ll * HashR[r+1].r * pwr(prime[1],r-l+1,mod[1]) ) % mod[1];
	if(hashp.second < 0)
		hashp.second += mod[1];
	mypair FH;
	FH.l = hashp.first;
